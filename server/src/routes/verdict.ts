...
import { z } from 'zod';
import { ACCENTS, type Witness } from '../domain/case.js';
import { districtLadder, newlyUnlocked, rewardFor } from '../domain/districts.js';
import { districtOpens } from '../domain/news.js';
...
const submitSchema = z.object({
  ...
  /**
   * How much of the file the juror read, reported by the client. Used ONLY by
   * missions ("open every exhibit before deciding"). It is self-reported and
   * low-stakes by design: the worst a forged report buys is a mission's XP,
   * never standing, never trust, and never anything about the verdict.
   */
  read: z
    .object({
      examined: z.number().int().min(0).max(3).default(0),
      witnesses: z.number().int().min(0).max(2).default(0),
      arguments: z.boolean().default(false),
    })
    .optional(),
});
...
verdictRouter.post('/', requireJuror, verdictLimiter, async (req, res) => {
  ...
  // Harder districts pay more for the same sitting. The seat is the one the
  // player is in now — the case was generated for it.
  const place = placeForUser(user);
  const reward = rewardFor(place.difficulty);
  ...
  // Missions see the whole sitting: what was read, where, and the city after.
  const echo = await prisma.character.count({
    where: {
      userId,
      originCaseId: { not: caseId },
      name: { in: [caseData.defendantName, ...(caseData.witnesses as Witness[]).map((w) => w.name)] },
    },
  });
  const ladderNow = districtLadder({ ...user, rank });
  await tickMissions(userId, {
    verdict,
    wasHung,
    timeRemaining,
    clockSeconds: CLOCK_SECONDS,
    mood: moodOf(caseData.accent),
    examined: parsed.data.read?.examined ?? 0,
    witnesses: parsed.data.read?.witnesses ?? 0,
    arguments: parsed.data.read?.arguments ?? false,
    difficulty: place.difficulty,
    echo: echo > 0,
    city: updatedCity,
    rank,
    streak: streak.streak,
    districtsOpen: ladderNow.filter((d) => d.unlocked).length,
  });

  // The papers. Printed now so the verdict screen can show the front page.
  const printed = await printVerdict(user, caseData, verdict, wasHung, effectKey, updatedCity);
  // A rank that opens a district is news too — the map just got bigger.
  const opened = newlyUnlocked(ladderNow, rankBefore, rank);
  if (opened.length) {
    await printStories(
      userId,
      place.district,
      opened.map((d) => districtOpens(place.district, d.name)),
    );
  }
  ...
  res.json({
    ...
    // The front page this verdict made, for the verdict screen's teaser.
...
/** The case's accent colour, back to its mood name, for missions. */
function moodOf(accent: string): string {
  const entry = Object.entries(ACCENTS).find(([, hex]) => hex.toLowerCase() === accent.toLowerCase());
  return entry?.[0] ?? accent;
}
